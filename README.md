...
| `run_openfoam.sh` and `run_openfoam_amr.sh` | OpenFOAM wrapper scripts that convert the Gmsh mesh into OpenFOAM polyMesh format, fix boundary patch types, run the chosen solver, and export the result to VTK. The two scripts differ only in which mesh file they read (`sim_mesh` for the baseline run, `sim_mesh_amr` for the AMR runs). |
